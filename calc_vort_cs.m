function [vort,z6t]=calc_vort_cs()
...
dxC = netcdf.getVar( ncid, netcdf.inqVarID( ncid, 'dxC' ), [ 0 0 ], [ nx+1 ny ] );
dyC = netcdf.getVar( ncid, netcdf.inqVarID( ncid, 'dyC' ), [ 0 0 ], [ nx ny+1 ] );
rAz = netcdf.getVar( ncid, netcdf.inqVarID( ncid, 'rAz' ), [ 0 0 ], [ nx+1 ny ] );

% Close the nc file.
netcdf.close( ncid );

% Open the nc file.
ncid = netcdf.open( '/hpcdata/scratch/am8e13/cs_36km_tutorial/run_tempcorrobcs/results/state.nc', 'NOWRITE' );

% Load the required fields.
u3d = netcdf.getVar( ncid, netcdf.inqVarID( ncid, 'U' ), [ 0 0 0 6 ], [ nx+1 ny nz 10 ] );
v3d = netcdf.getVar( ncid, netcdf.inqVarID( ncid, 'V' ), [ 0 0 0 6 ], [ nx ny+1 nz 10 ] );
...
nx=dims(1) 
nc=dims(2) 
ncp=nc+1; n2p=nc+2; nPg=nx*nc ;
%if nx == 210, flag=1; else flag=0; end
