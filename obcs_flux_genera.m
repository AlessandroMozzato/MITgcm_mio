...
cd /scratch/general/am8e13/cs_36km_tutorial/OB1992_2010_c81_36km/
...
for pos = ['N','E','W']
    for var = ['v','u']
        ...
        data1 = data ;
        [sumflux,flux] = flux_calc(data1,pos,var,dxdz_u,dxdz_v,dydz_u,dydz_v,nx,ny); 

%         while (abs(sumflux) > 5)
%             data1(data ~= 0) = data(data ~= 0 ) + (-1)*sign(sumflux)*corr_value ;
%             [sumflux,flux] = flux_calc(data1,pos,var,dxdz_u,dxdz_v,dydz_u,dydz_v,nx,ny) ;
%             sumflux = sumflux;
%             corr_value = corr_value + 0.0000001 ;
%             fprintf('%s: sumflux = %d, corr_value = %d \n', avenames{ind}, sumflux, corr_value)
%         end
% 
...
%     
         ...
         file_name_complete = strcat(file_name,'_mean') ;
